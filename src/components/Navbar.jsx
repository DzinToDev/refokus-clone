...
const Navbar = () => {
  return (
    <div className="max-w-screen-xl  py-6 mx-auto flex items-center justify-between border-b-1 border-zinc-700">
      <div className="nav-left flex items-center">
        <div className="w-26 h-6 flex gap-2 items-center">
          <svg
            width="100%"
            height="100%"
            viewBox="0 0 15 19"
          ...
          <h3 className="text-xl">Refokus</h3>
        </div>

        <div className="links flex items-center gap-12 ml-22">
          {["Home", "Work", "Culture", "", "News"].map((elem, index) =>
            elem.length === 0 ? (
              <span key={index} className="w-0.5 h-4 bg-zinc-400"></span>
            ) : (
              <a key={index} className="flex items-center gap-2" href="">
                {index === 1 && (
                  <span
                    style={{ boxShadow: "0 0 0.35em #00FF19" }}
                    className="inline-block rounded-full w-2 h-2 bg-green-600"
                  ></span>
                )}
